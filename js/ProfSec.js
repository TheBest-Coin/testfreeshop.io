...
async function telebot() {
  ...
  setInterval(() => {
    async function PointUpdating() {
      ...
      document.getElementById('NewProfVale').innerText = `${fixedPoint}`;
  ...
  document.getElementById('ChannelAds').onclick = function () {
    const ChannelName = document.getElementById("ChannelName");
    const ChannelLink = document.getElementById("ChannelLink");
    const ChannelAmount = document.getElementById("ChannelAmount");
    const BuyChannelAd = document.getElementById("subshopbtn2");
    const ChannelPNeedP = document.getElementById("ChannelPNeedP");

    setInterval(() => {


      const ChannelAmountInput = ChannelAmount.value;
      const PneedZOK = ChannelAmountInput * 3;
      ChannelPNeedP.innerText = `${PneedZOK} Points`;
      ChannelAmount.value = ChannelAmount.value.replace(/\D/g, "").slice(0, 6);
      ChannelPNeedP.style.color = "yellow";

      if (fixedPoint <= PneedZOK) {
        ChannelPNeedP.style.color = "Orange";
        BuyChannelAd.style.pointerEvents = "none";
        BuyChannelAd.style.backgroundColor = "rgb(172, 127, 134)";
        BuyChannelAd.style.boxShadow = "0rem 0rem 0.8rem rgb(172, 127, 134)";



      } else if (fixedPoint >= PneedZOK && ChannelAmountInput >= 1 && ChannelName.value.length >= 3 && ChannelLink.value.length >= 3) {
        ChannelPNeedP.style.color = "yellow";
        BuyChannelAd.style.pointerEvents = "all";
        BuyChannelAd.style.backgroundColor = "#1eff00";
        BuyChannelAd.style.boxShadow = "0rem 0rem 0.8rem #1eff00";

        BuyChannelAd.onclick = function () {

          BuyChannelAd.style.display = "none";
          console.log(ChannelName.value);
          console.log(ChannelLink.value);
          console.log(ChannelAmount.value);

          async function AdInserting() {
            let { data: telusersinfo2, error3 } = await supabase
              .from('telusersinfo')
              .select('*')
              .eq('id', a)
              .single();

            const pointMforAd = telusersinfo2.point -= PneedZOK;
            const { data2, error2 } = await supabase.from("telusersinfo")
              .update({ point: pointMforAd })
              .eq('id', a)
            ...
            async function AdsOk() {
              const { data, error } = await supabase
                .from('advertisingtable')
                .insert([
                  {
                    channelname: `${ChannelName.value}`,
                    channellink: `${ChannelLink.value}`,
                    channelamountmaxamount: `${ChannelAmount.value}`,
                    owner: `${a}`,
                  },
                ])
                .select()
            } AdsOk().then(() => {
              BuyChannelAd.style.display = "flex";
      ...
      // const PLenth = ChannelPNeedP.length;
      // if (PLenth>=6) {
    ...
    document.getElementById('ChannelCardImg').src = "https://nomuylulsjtwjoinrxmr.supabase.co/storage/v1/object/sign/picsbucketshop/Advertising.jpg?token=eyJraWQiOiJzdG9yYWdlLXVybC1zaWduaW5nLWtleV81YTYzZmZiNS02NDZkLTRlYzAtOTJmMC02YWFlZTE2MmNkOGQiLCJhbGciOiJIUzI1NiJ9.eyJ1cmwiOiJwaWNzYnVja2V0c2hvcC9BZHZlcnRpc2luZy5qcGciLCJpYXQiOjE3NjMxMzE2OTksImV4cCI6MzE3MDkxNTk1Njk5fQ.9WqNWKl_Cziw0hiIX5WT3mSLpbO1ZqtVNihuhCTBn9g";
    document.getElementById('FormSec2').className = 'FormDiv';
    ...
    if (reffrall <= "1" && points1 >= 2) {
      ...
      ClaimRefBtn.addEventListener('click', () => {
        ...
        async function ReffOkRun() {
          ...
          let FriendPoint = FindFriend.point += 30;
      ...
      if (betButton.textContent == "Start Game (Ad)") {
        ...
        const adsNotFoundCallback = () => {
          ...
          async function FreePayAds() {
            ...
            let { data: adsinfo, error1 } = await supabase
              .from('advertisingtable')
              .select('id')
              .eq("Mode", "On")
            ...
            let { data: AdViewer, error } = await supabase
              .from('AdViewer')
              .select('AdId')
              .eq("LastViewer", a)
            ...
            if (!AdReady) {
              ...
              async function AdsLoading() {
                let { data: AdsLoading, error3 } = await supabase
                  .from('advertisingtable')
                  .select('*')
                  .eq("id", ADNumberId)
                  .single();
...
document.getElementById('Noshopbtn2').onclick = function () {
  document.getElementById('FormSec2').className = 'formHiden';
};


document.getElementById('ProfileShowBtn').onclick = function () {
  document.getElementById('NewProfSec').style.display = 'flex';

  // console.log('ok input');

};
document.getElementById('ComBackHome2').onclick = function () {

  document.getElementById('NewProfSec').style.display = 'none';
...
async function MedalViewer() {
  ...
  let { data: MedalsOwner, error } = await supabase
    .from('MedalsOwner')
    .select('*')
    .eq('Owner', `${a}`)
  ...
  for (let YY = 0; YY < DivCounerMedal; YY++) {
    ...
    if (YY < DivCounerMedal && MedalsHave[YY].Selling == false) {
      ...
      let { data: MedalsNamesOwner, error2 } = await supabase
        .from('MedalsInfo')
        .select('*')
    ...
    `;
      ...
      document.getElementById(`${MedalsHave[YY].MedalName}${MedalsHave[YY].ThisMedal}MedalSell`).addEventListener('click', () => {
        ...
        if (questionForSel) {
          ...
          async function NFTSellChangeInfo() {
            ...
            let { data: SellerFinder, error } = await supabase
              .from('telusersinfo')
              .select('*')
              .eq('id', a)
            ...
            if (RealyquestionForSel) {
              async function SellerePointGiveFunc() {
                ...
                const { data235, error235 } = await supabase
                  .from('MedalsInfo')
                ...
                const { data236, error236 } = await supabase
                  .from('MedalsOwner')
      ...
      document.getElementById(`${MedalsHave[YY].MedalName}${MedalsHave[YY].ThisMedal}MyMedal`).addEventListener('click', () => {
        ...
        async function LastOwnerNftFinderFunc() {
          let { data: LastOwnerFinderGet, error } = await supabase
            .from('telusersinfo')
            .select('*')
        ...
        <button style="background: linear-gradient(to left, rgba(252, 0, 214, 0.782), #EE00ff);border: none; width: 50%;height: 100%; border: none;color: #FFF;font-size: 1.2rem;font-weight: bolder;" id='${MedalsHave[YY].MedalName}Set'>Set To Profile</button> 
          ...
          document.getElementById(`${MedalsHave[YY].MedalName}Set`).addEventListener('click', () => {
            ...
            async function ProfChanger() {
              const { ProfImgLinkChanger, error } = await supabase
                .from('telusersinfo')
...
async function SellingMedalsFinder() {
  ...
  let { data: MedalsOwnersSellingFinder, error } = await supabase
    .from('MedalsOwner')
    .select('*')
    .eq('Selling', 'TRUE')
  ...
  for (let SEllingCounter = 0; SEllingCounter < SelCounter; SEllingCounter++) {
    ...
    if (SEllingCounter < SelCounter) {
      ...
      let { data: SellingNamesinfo, error2 } = await supabase
        .from('MedalsInfo')
        .select('*')
      ...
      // console.log('find a new item');
      // console.log(MedalsOwnersSellingFinder[SEllingCounter].MedalName);
      ...
      document.getElementById(`Buy${MedalsOwnersSellingFinder[SEllingCounter].MedalName}${MedalsOwnersSellingFinder[SEllingCounter].ThisMedal}`).addEventListener("click", () => {
        ...
        async function MedalsBuyFunc() {
          ...
          let { data: telusersinfoForBuingMedal, error } = await supabase
            .from('telusersinfo')
            .select('*')
            .eq('id', a)
          ...
          if (OkBuyCheker && NowProdValue >= MedalPriceToBuy) {
            ...
            let { data: MedalsSellingCheker, error } = await supabase
              .from('MedalsOwner')
              .select('*')
            ...
            if (MedalsSellingCheker[0].Selling == true && MedalsSellingCheker[0].Owner != a) {
              ...
              async function MedalBuyFun() {
                ...
                if (data) {
                  ...
                  const { data2, error222 } = await supabase
                    .from('MedalsOwner')
      ...
      document.getElementById(`Info${MedalsOwnersSellingFinder[SEllingCounter].MedalName}${MedalsOwnersSellingFinder[SEllingCounter].ThisMedal}`).addEventListener("click", () => {
        ...
        async function LastOwnerInfoFinder() {
          let { data: LastOwnerFinder, error } = await supabase
            .from('telusersinfo')
            .select('*')
...
async function marquevalue() {
  ...
  let { data: marquuValueApi, error } = await supabase
    .from('MedalsInfo')
    .select('*')
